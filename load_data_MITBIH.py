...
def label_clean_segments_q_points(record, annotation, sampfrom):
    ...
    if os.path.exists(f'./Q_points/{record.record_name}Q.dat') and sampfrom ==0:
        q_points = np.genfromtxt(f'./Q_points/{record.record_name}Q.dat').astype(int)
    else:
...
def load_mit_bih_data(name, sampfrom, sampto):
    ...
    # For basic plot
    annotation_basic = wfdb.rdann(f'./data/sample_{name}/{name}', 'atr', sampfrom=sampfrom, sampto=length,
                                  shift_samps=True)

    # plot_data_basic(record, annotation_basic)
