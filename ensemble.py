...
def mean_prime_2(signal, norm_power_range, win_length_range):
    expanded_data_mean = []
    win_length_range = [value for value in win_length_range if value > 1]
...
def summarise_scores(all_scores):
    ...
    for c in range(len(all_scores[0])):
        scores_per_point = np.array([score[c] for score in all_scores])
    ...
    return final_scores, heights, locations
...
def run(data, win_length_max, m):
    ...
    return summarise_scores(outlier_scores_m)
...
def run_NAB():
    ...
    scores, heights, locations = run(data, 100, m)
    print(scores[810], scores[811], scores[812], scores[813], scores[814], scores[815], scores[816])

    pc.all_plots(name, data, scores, labels, guesses, to_add_values, heights, locations, runs=m, type=type)
